...
public class DataManager {
     ...
     */
    private static final String SQL_INSERT_NEW_CUSTOMER = "insert into CUSTOMER (CUSTOMER_ID, FIRST_NAME, "
            + "LAST_NAME, STREET, ZIPCODE, CITY, VERSION) " + "values (?, ?, ?, ?, ?, ?, 0)";

    /**
     * prepared statement to query a customer by id: <code>{@value}</code>
     */
    private static final String SQL_SELECT_CUSTOMER_BY_ID = "select CUSTOMER_ID, FIRST_NAME, "
            + "LAST_NAME, STREET, ZIPCODE, CITY, VERSION from CUSTOMER " + "where CUSTOMER_ID=?";

    /**
     * prepared statement to update a customer and increase version: <code>{@value}</code>
     */
    private static final String SQL_UPDATE_CUSTOMER = "update CUSTOMER set FIRST_NAME=?, "
            + "LAST_NAME=?, STREET=?, ZIPCODE=?, CITY=?, VERSION=VERSION + 1 " + "where CUSTOMER_ID=? and VERSION=?";
     ...
     */
    public void addCustomer(int customerId, String firstName, String lastName, String street, String zipCode,
            String city) {
        try (Connection con = dataSource.getConnection();
                PreparedStatement stmt = con.prepareStatement(SQL_INSERT_NEW_CUSTOMER)) {
    ...
    private Customer findCustomerByIdInternal(int customerId) {
        Customer res = null;
        try (Connection con = dataSource.getConnection();
                PreparedStatement stmt = con.prepareStatement(SQL_SELECT_CUSTOMER_BY_ID)) {
            stmt.setInt(1, customerId);
            try (ResultSet rs = stmt.executeQuery()) {
                if (rs.next()) {
                    res = new Customer(customerId, rs.getString(2), rs.getString(3), rs.getString(4), rs.getString(5),
                            rs.getString(6));
    ...
    public Customer findCustomerById(int customerId) {
        LOGGER.fine(Thread.currentThread().getName() + ": " + DataManager.class.getSimpleName() + ".findCustomerById('"
                + customerId + "') called");
    ...
    public void storeCustomer(Customer customer) {
        LOGGER.fine(Thread.currentThread().getName() + ": " + DataManager.class.getSimpleName() + ".storeCustomer("
                + customer + ") called");

        LOGGER.fine(Thread.currentThread().getName() + ": "
                + "Performing transactional update on database, expecting record version: " + customer.getVersion());

        int numberOfUpdatedRecords = updateCustomer(customer);

        if (numberOfUpdatedRecords == 1) {
            customer.setVersion(customer.getVersion() + 1);
            LOGGER.fine(Thread.currentThread().getName() + ": " + "Database update successful, new record version: "
                    + customer.getVersion());
        }
        else {
            ...
            if (currentCustomerVersion == null) {
                LOGGER.fine(Thread.currentThread().getName() + ": "
                        + "Database update failed, record deletion detected, throwing exception");
                throw new ConcurrentModificationException("Customer could not be updated: " + customer + " (deleted)");
            }
            else {
                LOGGER.fine(Thread.currentThread().getName() + ": "
                        + "Database update failed, concurrent update detected (version mismatch, current version: "
                        + currentCustomerVersion.getVersion() + ", expected: " + customer.getVersion()
                        + "), throwing exception");
                throw new ConcurrentModificationException("Customer could not be updated: " + customer
                        + " (version mismatch, current version: " + currentCustomerVersion.getVersion()
    ...
    private int updateCustomer(Customer customer) {
        ...
        try (Connection con = dataSource.getConnection();
                PreparedStatement stmt = con.prepareStatement(SQL_UPDATE_CUSTOMER)) {
