...
public abstract class AbstractVoucher {
    ...
    public AbstractVoucher(String firstName, String lastName, double value) {
        if (firstName == null || firstName.trim().length() == 0 || lastName == null || lastName.trim().length() == 0
                || value <= 0) {
            throw new IllegalArgumentException("Arguments first name and last name "
                    + "must be specified, value must be > 0 (given: firstName=" + firstName + ", lastName=" + lastName
                    + ", value=" + value + ").");
        }
    ...
    public String toString() {
        ...
        return this.getClass().getSimpleName() + "({id=" + this.getId() + ", displayCode="
                + this.getVoucherDisplayCode() + ", firstName=" + this.getFirstName() + ", lastName="
                + this.getLastName() + ", value=" + this.getValue() + "})";
    }
}
