...
public class DocumentManagerSecurityProxy implements DocumentManager {
    ...
    public DocumentManagerSecurityProxy(DocumentManager instanceToBeProtected) {
        LOGGER.fine("Creating new " + this.getClass().getSimpleName() + " for "
                + instanceToBeProtected.getClass().getSimpleName());
    ...
    public String findDocumentByName(String documentName) {
        LOGGER.fine("findDocumentByName(...) called on " + this.getClass().getSimpleName()
                + " called - checking permission ...");
        if (SimpleAccessManager.getInstance().checkPermission()) {
            LOGGER.fine("Permission granted - Delegating findDocumentByName(...) to concrete document manager "
                    + protectedInstance.getClass().getSimpleName());
            return protectedInstance.findDocumentByName(documentName);
        }
        else {
            LOGGER.fine("Permission denied, call will not be delegated to concrete document manager "
                    + protectedInstance.getClass().getSimpleName());
    ...
    public void storeDocument(String documentName, String document) {
        LOGGER.fine("storeDocument(...) called on " + this.getClass().getSimpleName()
                + " called - checking permission ...");
        if (SimpleAccessManager.getInstance().checkPermission()) {
            LOGGER.fine("Permission granted - Delegating storeDocument(...) to concrete document manager "
                    + protectedInstance.getClass().getSimpleName());
